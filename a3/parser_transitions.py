...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for i, partial_parse in enumerate(unfinished_parses[:batch_size]):
            partial_parse.parse_step(transitions[i])

        unfinished_parses = [
            partial_parse
            for partial_parse in unfinished_parses[:batch_size]
            if len(partial_parse.stack) != 1 or len(partial_parse.buffer) != 0
